...
void OutputManager::printRatesToFile(const Neighbourlist &nbList, const Topology topol) {
  std::ofstream outFile;
  ...
  if (outFile.is_open()) {
    ...
    for (int i = 0; i < nbList.getNumberOfSites(); i++) {
      for (auto &nb : nbList.getLRNeighbours(i)) {
        outFile << boost::format("%.5e %.5e %d %d\n") %
                        nb.rate_s % nb.dr.norm() % topol.getMolType(i) % topol.getMolType(nb.nb);
      }
    } 
